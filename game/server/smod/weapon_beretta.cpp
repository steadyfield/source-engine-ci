...
#define	BERETTA_FASTEST_REFIRE_TIME		0.2f
...
#define	BERETTA_ACCURACY_MAXIMUM_PENALTY_TIME	1.5f	// Maximum penalty to deal out
	...
	{		
		...
		if ( pistol_use_new_accuracy.GetBool() )
		{
			float ramp = RemapValClamped(	m_flAccuracyPenalty, 
											0.0f, 
											BERETTA_ACCURACY_MAXIMUM_PENALTY_TIME, 
											0.0f, 
											1.0f ); 

			// We lerp from very accurate to inaccurate over time
			VectorLerp( vec3_origin, VECTOR_CONE_2DEGREES, ramp, cone );
		}
		else
		{
			// Old value
...
void CWeaponBeretta::DryFire( void )
{
	...
	m_flSoonestPrimaryAttack	= gpGlobals->curtime + BERETTA_FASTEST_DRY_REFIRE_TIME;
...
void CWeaponBeretta::PrimaryAttack( void )
{
	...
	m_flSoonestPrimaryAttack = gpGlobals->curtime + BERETTA_FASTEST_REFIRE_TIME;
	...
	m_flAccuracyPenalty += BERETTA_ACCURACY_SHOT_PENALTY_TIME;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponBeretta::UpdatePenaltyTime( void )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
	...
	if ( ( ( pOwner->m_nButtons & IN_ATTACK ) == false ) && ( m_flSoonestPrimaryAttack < gpGlobals->curtime ) )
	{
		...
		m_flAccuracyPenalty = clamp( m_flAccuracyPenalty, 0.0f, BERETTA_ACCURACY_MAXIMUM_PENALTY_TIME );
	}
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponBeretta::ItemPreFrame( void )
{
	UpdatePenaltyTime();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponBeretta::ItemBusyFrame( void )
{
	UpdatePenaltyTime();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponBeretta::AddViewKick( void )
{
...
void CWeaponBeretta::SecondaryAttack()
{
	...
	if (pPlayer->Weapon_OwnsThisType("weapon_beretta_akimbo"))
		pPlayer->Weapon_Switch(pPlayer->Weapon_OwnsThisType("weapon_beretta_akimbo"));
}
