...
LINK_ENTITY_TO_CLASS(prop_metal_ragdoll, CMetalRagdoll);


CMetalRagdoll::CMetalRagdoll()
{
...
void CMetalRagdoll::Precache()
{
}

void CMetalRagdoll::Spawn()
{
	Precache();
}

void CMetalRagdoll::TraceAttack(const CTakeDamageInfo &info, const Vector &dir, trace_t *ptr, CDmgAccumulator *pAccumulator)
{
	g_pEffects->MetalSparks( ptr->endpos, ptr->plane.normal );
}

BEGIN_DATADESC(CMetalRagdoll)
END_DATADESC()
...
CMetalRagdoll* CreateMetalServerRagdoll(CBaseAnimating* pAnimating, int forceBone, const CTakeDamageInfo& info, int collisionGroup, bool bUseLRURetirement)
{
	...
	CMetalRagdoll* pRagdoll = (CMetalRagdoll*)CBaseEntity::CreateNoSpawn("prop_metal_ragdoll", pAnimating->GetAbsOrigin(), vec3_angle, NULL);
