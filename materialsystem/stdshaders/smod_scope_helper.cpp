...
#include "smod_scope_helper.h"
#include "convar.h"
#include "smod_scope_vs20.inc"
#include "smod_scope_ps20.inc"
#include "smod_scope_ps20b.inc"
#include "cpp_shader_constant_register_map.h"

#define MAXBLUR 1

// FIXME: doesn't support fresnel!
void InitParamssmod_scope(CBaseVSShader *pShader, IMaterialVar** params, const char *pMaterialName, smod_scope_Vars_t &info)
{
	...
	IShaderShadow* pShaderShadow, smod_scope_Vars_t &info, VertexCompressionType_t vertexCompression)
{
	...
	SHADOW_STATE
	{
		...
		DECLARE_STATIC_VERTEX_SHADER(smod_scope_vs20);
		SET_STATIC_VERTEX_SHADER_COMBO(MODEL,  bIsModel);
		SET_STATIC_VERTEX_SHADER_COMBO(COLORMODULATE, bColorModulate);
		SET_STATIC_VERTEX_SHADER(smod_scope_vs20);
		...
		{
			DECLARE_STATIC_PIXEL_SHADER(smod_scope_ps20b);
			...
			SET_STATIC_PIXEL_SHADER(smod_scope_ps20b);
		}
		else
		{
			DECLARE_STATIC_PIXEL_SHADER(smod_scope_ps20);
			SET_STATIC_PIXEL_SHADER_COMBO(BLUR,  blurAmount);
			SET_STATIC_PIXEL_SHADER_COMBO(FADEOUTONSILHOUETTE,  bFadeOutOnSilhouette);
			SET_STATIC_PIXEL_SHADER_COMBO(CUBEMAP,  bHasEnvmap);
			SET_STATIC_PIXEL_SHADER_COMBO(REFRACTTINTTEXTURE,  bRefractTintTexture);
			...
			SET_STATIC_PIXEL_SHADER_COMBO(SECONDARY_NORMAL, bSecondaryNormal);
			SET_STATIC_PIXEL_SHADER_COMBO(NORMAL_DECODE_MODE, (int)nNormalDecodeMode);
			SET_STATIC_PIXEL_SHADER(smod_scope_ps20);
		}
		pShader->DefaultFog();
		if (bMasked)
		{
		...
		DYNAMIC_STATE
	{
		...
		DECLARE_DYNAMIC_VERTEX_SHADER(smod_scope_vs20);
		SET_DYNAMIC_VERTEX_SHADER_COMBO(SKINNING,  pShaderAPI->GetCurrentNumBones() > 0);
		SET_DYNAMIC_VERTEX_SHADER_COMBO(COMPRESSED_VERTS, (int)vertexCompression);
		SET_DYNAMIC_VERTEX_SHADER(smod_scope_vs20);
		...
		{
			DECLARE_DYNAMIC_PIXEL_SHADER(smod_scope_ps20b);
			...
			SET_DYNAMIC_PIXEL_SHADER(smod_scope_ps20b);
		}
		else
		{
			DECLARE_DYNAMIC_PIXEL_SHADER(smod_scope_ps20);
			SET_DYNAMIC_PIXEL_SHADER_COMBO(PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo());
			SET_DYNAMIC_PIXEL_SHADER(smod_scope_ps20);
		}
