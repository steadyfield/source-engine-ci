...
STUB_WEAPON_CLASS( weapon_stickybomb, WeaponStickyLauncher, C_BaseHLCombatWeapon );
	...
	int InternalDrawModel( int flags )
	{
		if ( IsFollowingEntity() && m_boneIndexAttached >= 0 )
		{
			...
			if ( follow )
			{
				...
				Vector absOrigin;
				MatrixGetColumn( m_CachedBones, 3, absOrigin );
