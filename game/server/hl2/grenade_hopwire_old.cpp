...
public:
	...
	static CTetherHook	*Create( const Vector &origin, const QAngle &angles, CGrenadeHopWireOld *pOwner );
...
private:
	CHandle<CGrenadeHopWireOld>	m_hTetheredOwner;
...
//			*pOwner - 
//-----------------------------------------------------------------------------
CTetherHook	*CTetherHook::Create( const Vector &origin, const QAngle &angles, CGrenadeHopWireOld *pOwner )
{
...
BEGIN_DATADESC( CGrenadeHopWireOld )
...
LINK_ENTITY_TO_CLASS( npc_grenade_hopwire_old, CGrenadeHopWireOld );

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CGrenadeHopWireOld::Spawn( void )
{
	Precache();
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CGrenadeHopWireOld::CreateVPhysics()
{
	// Create the object in the physics system
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGrenadeHopWireOld::Precache( void )
{
...
// Input  : timer - 
//-----------------------------------------------------------------------------
void CGrenadeHopWireOld::SetTimer( float timer )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGrenadeHopWireOld::CombatThink( void )
{
	if ( m_pGlow == NULL )
	{
		m_pGlow = CSprite::SpriteCreate( "sprites/blueflare1.vmt", GetLocalOrigin(), false );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGrenadeHopWireOld::TetherThink( void )
{
	...
	if ( m_nHooksShot == 8 )
	{
		...
		SetThink(&CGrenadeHopWireOld::CombatThink);
		SetNextThink( gpGlobals->curtime + 0.1f );
	}
	else
	{
		SetThink(&CGrenadeHopWireOld::TetherThink);
...
//			&angVelocity - 
//-----------------------------------------------------------------------------
void CGrenadeHopWireOld::SetVelocity( const Vector &velocity, const AngularImpulse &angVelocity )
{
	IPhysicsObject *pPhysicsObject = VPhysicsGetObject();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGrenadeHopWireOld::Detonate( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
CBaseGrenade *HopWireOld_Create( const Vector &position, const QAngle &angles, const Vector &velocity, const AngularImpulse &angVelocity, CBaseEntity *pOwner, float timer )
{
	CGrenadeHopWireOld *pGrenade = (CGrenadeHopWireOld *) CBaseEntity::Create( "npc_grenade_hopwire_old", position, angles, pOwner );
