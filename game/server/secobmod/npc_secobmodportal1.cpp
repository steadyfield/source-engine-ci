...
//
// Purpose: SecobModportal1s act as targets for other NPC's to attack and to trigger
//			events 
//
// $NoKeywords: $
//=============================================================================//
...
class CSecobModportal1List : public CAutoGameSystem
{
public:
	CSecobModportal1List( char const *name ) : CAutoGameSystem( name )
	{
	}
	...
	void AddToList( CNPC_SecobModportal1 *pSecobModportal1 );
	void RemoveFromList( CNPC_SecobModportal1 *pSecobModportal1 );

	CUtlVector< CNPC_SecobModportal1 * >	m_list;
};

void CSecobModportal1List::AddToList( CNPC_SecobModportal1 *pSecobModportal1 )
{
	m_list.AddToTail( pSecobModportal1 );
}
...
CSecobModportal1List g_SecobModportal1List( "CSecobModportal1List" );
...
int FindSecobModportal1sInCone( CBaseEntity **pList, int listMax, const Vector &coneOrigin, const Vector &coneAxis, float coneAngleCos, float coneLength )
{
	...
	for ( int i = g_SecobModportal1List.m_list.Count() - 1; i >= 0; --i )
	{
		CNPC_SecobModportal1 *pTest = g_SecobModportal1List.m_list[i];
...
ConVar	sk_SecobModportal1_health( "sk_SecobModportal1_health","0");

BEGIN_DATADESC( CNPC_SecobModportal1 )
	...
	// Function Pointers
	DEFINE_THINKFUNC( SecobModportal1Think ),
...
LINK_ENTITY_TO_CLASS( npc_SecobModportal1, CNPC_SecobModportal1 );
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CNPC_SecobModportal1::CNPC_SecobModportal1( void )
{
	m_takedamage	= DAMAGE_YES;
	m_iHealth		= sk_SecobModportal1_health.GetFloat();
	m_hPainPartner	= NULL;
	g_SecobModportal1List.AddToList( this );
	m_flFieldOfView = 360;
...
CNPC_SecobModportal1::~CNPC_SecobModportal1( void )
{
	g_SecobModportal1List.RemoveFromList( this );
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_SecobModportal1::Precache( void )
{
	BaseClass::Precache();
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_SecobModportal1::Activate( void )
{
	BaseClass::Activate();

	if ( m_spawnflags & SF_SecobModportal1_PERFECTACC )
	{
...
//------------------------------------------------------------------------------
void CNPC_SecobModportal1::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if( GetParent() )
	{
		...
		{
			// If this SecobModportal1 is parented to a combine ball, explode the combine ball
			// and remove this SecobModportal1.
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_SecobModportal1::DecalTrace( trace_t *pOldTrace, char const *decalName )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_SecobModportal1::ImpactTrace( trace_t *pTrace, int iDamageType, char *pCustomImpactName )
{
	// Get direction of original trace
...
// Output : 
//-----------------------------------------------------------------------------
Class_T	CNPC_SecobModportal1::Classify( void )
{
	return	CLASS_NONE;
}

void CNPC_SecobModportal1::OnRestore( void )
{
	if ( m_spawnflags & SF_SecobModportal1_VPHYSICSSHADOW )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_SecobModportal1::SecobModportal1Think( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_SecobModportal1::CanBecomeRagdoll()
{
	return false;
}
...
//-----------------------------------------------------------------------------
// Purpose: SecobModportal1s should always report light damage if any amount of damage is taken
// Input  : fDamage - amount of damage
//			bitsDamageType - damage type
//-----------------------------------------------------------------------------
bool CNPC_SecobModportal1::IsLightDamage( const CTakeDamageInfo &info )
{
...
//			bitsDamageType - 
//-----------------------------------------------------------------------------
void CNPC_SecobModportal1::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
	//If specified, we must be the enemy of the target
	if ( m_spawnflags & SF_SecobModportal1_ENEMYDAMAGEONLY )
	{
	...
	if ( ( m_spawnflags & SF_SecobModportal1_BLEED ) && ( m_takedamage == DAMAGE_NO ) )
	{
...
// Output : int
//-----------------------------------------------------------------------------
int CNPC_SecobModportal1::OnTakeDamage( const CTakeDamageInfo &info )
{
...
// Input  : *pOther - 
//-----------------------------------------------------------------------------
void CNPC_SecobModportal1::SetPainPartner( CBaseEntity *pOther )
{
	m_hPainPartner = pOther;
}

void CNPC_SecobModportal1::InputTargeted( inputdata_t &inputdata )
{
	m_OnTargeted.FireOutput( inputdata.pActivator, inputdata.pCaller, 0 );
}

void CNPC_SecobModportal1::InputReleased( inputdata_t &inputdata )
{
