...
//
// Purpose: GET YOUR FREE GOD DAMN TV
//
//=============================================================================//
...
ConVar	sk_freetv_health( "sk_freetv_health","0");

envelopePoint_t envFreeTVMoanVolumeFast[] =
{
	{	7.0f, 7.0f,
		0.1f, 0.1f,
	},
...
envelopePoint_t envFreeTVMoanVolume[] =
{
	{	1.0f, 1.0f,
		0.1f, 0.1f,
	},
...
envelopePoint_t envFreeTVMoanVolumeLong[] =
{
	{	1.0f, 1.0f,
		0.3f, 0.5f,
	},
...
envelopePoint_t envFreeTVMoanIgnited[] =
{
	{	1.0f, 1.0f,
		0.5f, 1.0f,
	},
...
class CFreeTV : public CAI_BlendingHost<CNPC_BaseZombie>
{
	DECLARE_DATADESC();
	DECLARE_CLASS( CFreeTV, CAI_BlendingHost<CNPC_BaseZombie> );
...
public:
	...
	{
		//FreeTV "Zombie" has no headcrab
		return RELEASE_NO;
	}

	Class_T Classify()
	{
		return CLASS_I_HATE_EVERYONE;
	}

	bool ShouldBecomeTorso()
	{
		//FreeTV "Zombie" can not be cut in half
		return false;
	}
...
LINK_ENTITY_TO_CLASS( npc_freetv, CFreeTV );

//---------------------------------------------------------
//---------------------------------------------------------
const char *CFreeTV::pMoanSounds[] =
{
	 "NPC_FreeTV.Moan1",
	 "NPC_FreeTV.Moan2",
	 "NPC_FreeTV.Moan3",
...
enum
{
	SCHED_FREETV_BASH_DOOR = LAST_BASE_ZOMBIE_SCHEDULE,
	SCHED_FREETV_WANDER_ANGRILY,
	SCHED_FREETV_CHARGE_ENEMY,
	SCHED_FREETV_FAIL,
};
...
enum
{
	TASK_FREETV_EXPRESS_ANGER = LAST_BASE_ZOMBIE_TASK,
	TASK_FREETV_YAW_TO_DOOR,
	TASK_FREETV_ATTACK_DOOR,
	TASK_FREETV_CHARGE_ENEMY,
};
...
// Purpose: 
//-----------------------------------------------------------------------------
void CFreeTV::Precache( void )
{
	BaseClass::Precache();

	PrecacheModel( "models/zombie/freetv.mdl" );
	...
	PrecacheScriptSound( "NPC_FreeTV.Pain" );
	PrecacheScriptSound( "NPC_FreeTV.Die" );
	PrecacheScriptSound( "NPC_FreeTV.Alert" );
	PrecacheScriptSound( "NPC_FreeTV.Idle" );
	PrecacheScriptSound( "NPC_FreeTV.Attack" );

	PrecacheScriptSound( "NPC_FreeTV.Moan1" );
	PrecacheScriptSound( "NPC_FreeTV.Moan2" );
	PrecacheScriptSound( "NPC_FreeTV.Moan3" );
	PrecacheScriptSound( "NPC_FreeTV.Moan4" );
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CFreeTV::Spawn( void )
{
	Precache();
	...
	m_iHealth			= sk_freetv_health.GetFloat();
	m_flFieldOfView		= 0.2;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CFreeTV::PrescheduleThink( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CFreeTV::SelectSchedule ( void )
{
...
//-----------------------------------------------------------------------------
void CFreeTV::FootstepSound( bool fRightFoot )
{
	if( fRightFoot )
	{
...
//-----------------------------------------------------------------------------
void CFreeTV::FootscuffSound( bool fRightFoot )
{
	if( fRightFoot )
	{
...
//-----------------------------------------------------------------------------
void CFreeTV::AttackHitSound( void )
{
...
//-----------------------------------------------------------------------------
void CFreeTV::AttackMissSound( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CFreeTV::PainSound( const CTakeDamageInfo &info )
{
	...
	EmitSound( "NPC_FreeTV.Pain" );
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CFreeTV::DeathSound( const CTakeDamageInfo &info ) 
{
	EmitSound( "NPC_FreeTV.Die" );
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CFreeTV::AlertSound( void )
{
	EmitSound( "NPC_FreeTV.Alert" );
...
//-----------------------------------------------------------------------------
const char *CFreeTV::GetMoanSound( int nSound )
{
...
//-----------------------------------------------------------------------------
void CFreeTV::AttackSound( void )
{
	EmitSound( "NPC_FreeTV.Attack" );
}

//-----------------------------------------------------------------------------
// Purpose: Returns the classname (ie "npc_headcrab") to spawn when our headcrab bails.
//-----------------------------------------------------------------------------
const char *CFreeTV::GetHeadcrabClassname( void )
{
	return "npc_headcrab";
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
const char *CFreeTV::GetHeadcrabModel( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
const char *CFreeTV::GetLegsModel( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
const char *CFreeTV::GetTorsoModel( void )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CFreeTV::SetZombieModel( void )
{
	Hull_t lastHull = GetHullType();

	SetModel( "models/zombie/freetv.mdl" );
...
//---------------------------------------------------------
void CFreeTV::MoanSound( envelopePoint_t *pEnvelope, int iEnvelopeSize )
{
	if( IsOnFire() )
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CFreeTV::ShouldBecomeTorso( const CTakeDamageInfo &info, float flDamageThreshold )
{
	if( IsSlumped() ) 
	{
	...
	return false; //FreeTV "Zombie" is only 1 model
}

//---------------------------------------------------------
//---------------------------------------------------------
void CFreeTV::GatherConditions( void )
{
...
int CFreeTV::SelectFailSchedule( int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode )
{
	if ( HasCondition( COND_BLOCKED_BY_DOOR ) && m_hBlockingDoor != NULL )
	{
		ClearCondition( COND_BLOCKED_BY_DOOR );
		if ( m_NextTimeToStartDoorBash.Expired() && failedSchedule != SCHED_FREETV_BASH_DOOR )
			return SCHED_FREETV_BASH_DOOR;
		m_hBlockingDoor = NULL;
	}

	if ( failedSchedule != SCHED_FREETV_CHARGE_ENEMY && 
		 IsPathTaskFailure( taskFailCode ) &&
		 random->RandomInt( 1, 100 ) < 50 )
	{
		return SCHED_FREETV_CHARGE_ENEMY;
	}

	if ( failedSchedule != SCHED_FREETV_WANDER_ANGRILY &&
		 ( failedSchedule == SCHED_TAKE_COVER_FROM_ENEMY || 
		   failedSchedule == SCHED_CHASE_ENEMY_FAILED ) )
	{
		return SCHED_FREETV_WANDER_ANGRILY;
	}
...
Activity CFreeTV::NPC_TranslateActivity( Activity newActivity )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CFreeTV::OnStateChange( NPC_STATE OldState, NPC_STATE NewState )
{
...
void CFreeTV::StartTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
	case TASK_FREETV_EXPRESS_ANGER:
		{
...
void CFreeTV::RunTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
	case TASK_FREETV_ATTACK_DOOR:
...
bool CFreeTV::OnObstructingDoor( AILocalMoveGoal_t *pMoveGoal, CBaseDoor *pDoor, 
...
Activity CFreeTV::SelectDoorBash()
{
...
//---------------------------------------------------------
void CFreeTV::Ignite( float flFlameLifetime, bool bNPCOnly, float flSize, bool bCalledByLevelDesigner )
{
 	if( !IsOnFire() && IsAlive() )
	{
		...
		if ( !UTIL_IsLowViolence() )
		{
			...
			MoanSound( envFreeTVMoanIgnited, ARRAYSIZE( envFreeTVMoanIgnited ) );
...
//---------------------------------------------------------
//---------------------------------------------------------
int CFreeTV::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
#ifndef HL2_EPISODIC
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CFreeTV::IsHeavyDamage( const CTakeDamageInfo &info )
{
#ifdef HL2_EPISODIC
...
bool CFreeTV::IsSquashed( const CTakeDamageInfo &info )
{
	if( GetHealth() > 0 )
	{
		return false;
...
//---------------------------------------------------------
//---------------------------------------------------------
void CFreeTV::BuildScheduleTestBits( void )
{
...
AI_BEGIN_CUSTOM_NPC( npc_freetv, CFreeTV )
	...
	DECLARE_TASK( TASK_FREETV_EXPRESS_ANGER )
	DECLARE_TASK( TASK_FREETV_YAW_TO_DOOR )
	DECLARE_TASK( TASK_FREETV_ATTACK_DOOR )
	DECLARE_TASK( TASK_FREETV_CHARGE_ENEMY )
