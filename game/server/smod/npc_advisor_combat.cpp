...
#include "NPC_AdvisorCombat_shared.h"
...
#if 1 //NPC_AdvisorCombat_HAS_BEHAVIOR
...
// The advisor class.
//-----------------------------------------------------------------------------
class CNPC_AdvisorCombat : public CAI_BaseNPC
{
	DECLARE_CLASS( CNPC_AdvisorCombat, CAI_BaseNPC );
...
LINK_ENTITY_TO_CLASS( NPC_AdvisorCombat, CNPC_AdvisorCombat );

BEGIN_DATADESC( CNPC_AdvisorCombat )
	...
	DEFINE_ARRAY( m_haRecentlyThrownObjects, FIELD_EHANDLE, CNPC_AdvisorCombat::kMaxThrownObjectsTracked ),
	DEFINE_ARRAY( m_flaRecentlyThrownObjectTimes, FIELD_TIME, CNPC_AdvisorCombat::kMaxThrownObjectsTracked ),
...
#if 1 //NPC_AdvisorCombat_HAS_BEHAVIOR
IMPLEMENT_SERVERCLASS_ST(CNPC_AdvisorCombat, DT_NPC_AdvisorCombat)
...
void CNPC_AdvisorCombat::Activate()
{
	BaseClass::Activate();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::UpdateOnRemove()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::OnRestore()
{
	BaseClass::OnRestore();
...
//-----------------------------------------------------------------------------
Class_T	CNPC_AdvisorCombat::Classify()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_AdvisorCombat::IsHeavyDamage( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::StartLevitatingObjects()
{
	if ( !m_pLevitateController )
	{
...
bool CNPC_AdvisorCombat::CanLevitateEntity( CBaseEntity *pEntity, int minMass, int maxMass )
{
...
CBaseEntity *CNPC_AdvisorCombat::ThrowObjectPrepare()
{
	...
	if (pThrowable)
	{
		...
		EmitSound( "NPC_AdvisorCombat.ObjectChargeUp" );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
...
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::RunTask( const Task_t *pTask )
{
...
// advisor_staging_num
		...
		case TASK_ADVISOR_BARRAGE_OBJECTS:
		{
			...
			if (gpGlobals->curtime > m_flThrowPhysicsTime)
			{
				...
				EmitSound( "NPC_AdvisorCombat.Blast" );
...
//----------------------------------------------------------------------------------------------
void CNPC_AdvisorCombat::Event_Killed(const CTakeDamageInfo &info)
{
...
//-----------------------------------------------------------------------------
CBaseEntity *CNPC_AdvisorCombat::PickThrowable( bool bRequireInView )
{
...
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::HurlObjectAtPlayer( CBaseEntity *pEnt, const Vector &leadVel )
{
	IPhysicsObject *pPhys = pEnt->VPhysicsGetObject();
...
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::PreHurlClearTheWay( CBaseEntity *pThrowable, const Vector &toPos )
{
...
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::PurgeThrownObjects() 
{
...
//-----------------------------------------------------------------------------
bool CNPC_AdvisorCombat::DidThrow(const CBaseEntity *pEnt)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::AddToThrownObjects(CBaseEntity *pEnt)
{
	Assert(pEnt);
...
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::PullObjectToStaging( CBaseEntity *pEnt, const Vector &stagingPos )
{
	IPhysicsObject *pPhys = pEnt->VPhysicsGetObject();
	Assert(pPhys);
...
int	CNPC_AdvisorCombat::OnTakeDamage( const CTakeDamageInfo &info )
{
	// Clip our max 
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_AdvisorCombat::MeleeAttack1Conditions(float flDot, float flDist)
{
	if (flDist > 128)
	{
...
//-----------------------------------------------------------------------------
int CNPC_AdvisorCombat::SelectSchedule()
{
    if ( IsInAScript() )
...
void CNPC_AdvisorCombat::HandleAnimEvent(animevent_t *pEvent)
{
	switch (pEvent->event)
	{
...
//-----------------------------------------------------------------------------
Vector CNPC_AdvisorCombat::GetThrowFromPos( CBaseEntity *pEnt )
{
	Assert(pEnt);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::Precache()
{
	BaseClass::Precache();
	...
	PrecacheScriptSound( "NPC_AdvisorCombat.Blast" );
	PrecacheScriptSound( "BaseCombatCharacter.CorpseGib" );	//NPC_AdvisorCombat.Gib
	PrecacheScriptSound( "NPC_AdvisorCombat.Speak" );	//NPC_AdvisorCombat.Idle
	PrecacheScriptSound( "NPC_AdvisorCombat.ScreenVx02" );	//NPC_AdvisorCombat.Alert
	PrecacheScriptSound( "NPC_AdvisorCombat.Scream" ); //NPC_AdvisorCombat.Die
	PrecacheScriptSound( "NPC_AdvisorCombat.Pain" );
	PrecacheScriptSound( "NPC_AdvisorCombat.ObjectChargeUp" );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::IdleSound()
{
	EmitSound( "NPC_AdvisorCombat.Speak" );
}


void CNPC_AdvisorCombat::AlertSound()
{
	EmitSound( "NPC_AdvisorCombat.ScreenVx02" );
}


void CNPC_AdvisorCombat::PainSound( const CTakeDamageInfo &info )
{
	EmitSound( "NPC_AdvisorCombat.Pain" );
}


void CNPC_AdvisorCombat::DeathSound( const CTakeDamageInfo &info )
{
	EmitSound( "NPC_AdvisorCombat.Scream" );
}


//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_AdvisorCombat::DrawDebugTextOverlays()
{
...
//-----------------------------------------------------------------------------
int CNPC_AdvisorCombat::GetSoundInterests()
{
...
//-----------------------------------------------------------------------------
bool CNPC_AdvisorCombat::QueryHearSound( CSound *pSound )
{
...
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::InputSetThrowRate( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::InputSetStagingNum( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::InputPinPlayer( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::StopPinPlayer(inputdata_t &inputdata)
{
	m_hPlayerPinPos = NULL;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::OnScheduleChange( void )
{
	Write_AllBeamsOff();
	m_hvStagedEnts.RemoveAll();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::GatherConditions( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::InputWrenchImmediate( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::Write_BeamOn(  CBaseEntity *pEnt )
{
	Assert( pEnt );
...
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::Write_BeamOff( CBaseEntity *pEnt )
{
	Assert( pEnt );
	EntityMessageBegin( this, true );
...
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::Write_AllBeamsOff( void )
{
	EntityMessageBegin( this, true );
...
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::InputTurnBeamOn( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
void CNPC_AdvisorCombat::InputTurnBeamOff( inputdata_t &inputdata )
{
	// inputdata should specify a target
	CBaseEntity *pTarget = gEntList.FindEntityByName( NULL, inputdata.value.StringID() );
	if ( pTarget )
...
void CNPC_AdvisorCombat::InputElightOn( inputdata_t &inputdata )
{
	EntityMessageBegin( this, true );
...
void CNPC_AdvisorCombat::InputElightOff( inputdata_t &inputdata )
{
	EntityMessageBegin( this, true );
...
CAdvisorLevitate::simresult_e	CAdvisorLevitate::Simulate( IPhysicsMotionController *pController, IPhysicsObject *pObject, float deltaTime, Vector &linear, AngularImpulse &angular )
{
	...
	CNPC_AdvisorCombat *pAdvisor = static_cast<CNPC_AdvisorCombat *>(m_Advisor.Get());
	Assert(pAdvisor);
...
//-----------------------------------------------------------------------------
const impactdamagetable_t &CNPC_AdvisorCombat::GetPhysicsImpactDamageTable( void )
{
...
//
//-----------------------------------------------------------------------------
AI_BEGIN_CUSTOM_NPC( NPC_AdvisorCombat, CNPC_AdvisorCombat )
