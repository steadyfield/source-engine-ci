...
void CCrossbowBolt_HL1::BoltTouch( CBaseEntity *pOther )
{
	...
	if ( pOther->m_takedamage != DAMAGE_NO )
	{
		...
		if ( pOther->IsPlayer() )
		{
            float m_fDamage = sk_plr_dmg_crossbow.GetFloat() * g_pGameRules->GetDamageMultiplier();
		...
		{
			CTakeDamageInfo	dmgInfo( this, GetOwnerEntity(), sk_npc_dmg_crossbow.GetFloat() * g_pGameRules->GetDamageMultiplier(), DMG_BULLET | DMG_NEVERGIB );
			CalculateMeleeDamageForce( &dmgInfo, vecNormalizedVel, tr.endpos );
			pOther->DispatchTraceAttack( dmgInfo, vecNormalizedVel, &tr );
		}
...
void CCrossbowBolt_HL1::ExplodeThink( void )
{
    ...
    CTakeDamageInfo	dmgInfo( this, GetOwnerEntity(), sk_npc_dmg_crossbow.GetFloat() * g_pGameRules->GetDamageMultiplier(), DMG_BLAST );
