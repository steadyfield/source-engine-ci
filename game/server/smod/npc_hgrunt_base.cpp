...
#define COMBINE_GRENADE_THROW_SPEED 700 //650
#define COMBINE_GRENADE_TIMER		3.5
#define COMBINE_GRENADE_FLUSH_TIME	14.0		// Don't try to flush an enemy who has been out of sight for longer than this. 3.0
#define COMBINE_GRENADE_FLUSH_DIST	320.0	// Don't try to flush an enemy who has moved farther than this distance from the last place I saw him. 256

#define COMBINE_LIMP_HEALTH				20
#define	COMBINE_MIN_GRENADE_CLEAR_DIST	500 //250
...
#define COMBINE_SHOTGUN_STANDING_POSITION	Vector( 0, 0, 36 ) //36 
#define COMBINE_SHOTGUN_CROUCHING_POSITION	Vector( 0, 0, 36 ) //36
...
extern string_t	s_iszShotgunClassname;

//-----------------------------------------------------------------------------
// Interactions
//-----------------------------------------------------------------------------
int	i_interactionCombineBash = 0; // melee bash attack
...
extern int AE_COMBINE_START_DEPLOY;
...
// Combine activities
//=========================================================
//extern Activity ACT_COMBINE_STANDING_SMG1;
//extern Activity ACT_COMBINE_CROUCHING_SMG1;
//extern Activity ACT_COMBINE_STANDING_AR2;
//extern Activity ACT_COMBINE_CROUCHING_AR2;
//extern Activity ACT_COMBINE_WALKING_AR2;
//extern Activity ACT_COMBINE_STANDING_SHOTGUN;
//extern Activity ACT_COMBINE_CROUCHING_SHOTGUN;
extern Activity ACT_COMBINE_THROW_GRENADE;
extern Activity ACT_COMBINE_LAUNCH_GRENADE;
extern Activity ACT_COMBINE_BUGBAIT;
extern Activity ACT_COMBINE_AR2_ALTFIRE;
extern Activity ACT_WALK_EASY;
extern Activity ACT_WALK_MARCH;
extern Activity ACT_STRIKER_JUMP;
extern Activity ACT_STRIKER_LAND;
extern Activity ACT_STRIKER_GLIDE;
// -----------------------------------------------
//	> Squad slots
// -----------------------------------------------
enum SquadSlot_T
{
	...
	SQUAD_SLOT_OVERWATCH2,
	SQUAD_SLOT_OVERWATCH3,
	SQUAD_SLOT_OVERWATCH4,
	SQUAD_SLOT_COMBINE_FLANK_FIRST,
	SQUAD_SLOT_COMBINE_FLANK_LAST = SQUAD_SLOT_COMBINE_FLANK_FIRST,
};
...
LINK_ENTITY_TO_CLASS(NPC_HGruntBase, CNPC_HGruntBase);
...
DEFINE_FIELD(m_flNextGrenadeRunCheck, FIELD_TIME),
DEFINE_FIELD(m_flNextDeployCheck, FIELD_TIME),
...
DEFINE_FIELD(CombineSpeed, FIELD_FLOAT),
...
DEFINE_FIELD(m_iSoldierdmg, FIELD_INTEGER),
...
bool CNPC_HGruntBase::CreateComponents()
{
	...
	m_Sentences.Init(this, "NPC_HGruntBase.SentenceParameters");
	return true;
}
...
void CNPC_HGruntBase::Precache()
{
	...
	PrecacheScriptSound("NPC_HGruntBase.GrenadeLaunch");
	PrecacheScriptSound("NPC_HGruntBase.WeaponBash");
...
void CNPC_HGruntBase::Spawn(void)
{
	...
	m_flFieldOfView = 0.3;// indicates the width of this NPC's forward view cone ( as a dotproduct result )
	m_NPCState = NPC_STATE_NONE;
	m_flNextGrenadeCheck = gpGlobals->curtime + 1;
	m_flNextGrenadeRunCheck = gpGlobals->curtime + 1;
	m_flNextPainSoundTime = 0;
	m_flNextAlertSoundTime = 0;
	m_bShouldPatrol = false;
	m_flNextFlinchTime = .1;
...
void CNPC_HGruntBase::GatherConditions()
{
	...
	{
		if (IsCurSchedule(SCHED_HECUGRUNT_WAIT_IN_COVER, false))
		{
...
void CNPC_HGruntBase::PrescheduleThink()
{
	...
	if (GetLastDamageTime() + 0.75f <= gpGlobals->curtime)
	{
		m_iSoldierdmg = 0;
	}
...
void CNPC_HGruntBase::DelaySquadAltFireAttack(float flDelay)
{
	...
	while (pSquadmate)
	{
		CNPC_HGruntBase *pCombine = dynamic_cast<CNPC_HGruntBase*>(pSquadmate);
...
bool CNPC_HGruntBase::ShouldMoveAndShoot()
{
	...
	if (IsCurSchedule(SCHED_HECUGRUNT_HIDE_AND_RELOAD, false))
	...
	if (IsCurSchedule(SCHED_HECUGRUNT_TAKE_COVER_FROM_BEST_SOUND, false))
		return false;

	if (IsCurSchedule(SCHED_HECUGRUNT_RUN_AWAY_FROM_BEST_SOUND, false))
		return false;

	if (HasCondition(COND_NO_PRIMARY_AMMO, false))
		m_flStopMoveShootTime = gpGlobals->curtime + random->RandomFloat(0.4f, 0.6f);

	if (m_pSquad && IsCurSchedule(SCHED_HECUGRUNT_TAKE_COVER1, false))
		m_flStopMoveShootTime = gpGlobals->curtime + random->RandomFloat(0.4f, 0.6f);
...
bool CNPC_HGruntBase::IsCurTaskContinuousMove()
{
	...
	if (pTask && (pTask->iTask == TASK_HECUGRUNT_CHASE_ENEMY_CONTINUOUSLY))
		return true;
...
{
	// NOTE: This reset is required because we change it in TASK_HECUGRUNT_CHASE_ENEMY_CONTINUOUSLY
	m_MoveAndShootOverlay.SetInitialDelay(0.75);

	switch (pTask->iTask)
	{
	case TASK_HECUGRUNT_SET_STANDING:
	{
	...
	case TASK_HECUGRUNT_CHASE_ENEMY_CONTINUOUSLY:
		StartTaskChaseEnemyContinuously(pTask);
		break;

	case TASK_HECUGRUNT_PLAY_SEQUENCE_FACE_ALTFIRE_TARGET:
		SetIdealActivity((Activity)(int)pTask->flTaskData);
		GetMotor()->SetIdealYawToTargetAndUpdate(m_vecAltFireTarget, AI_KEEP_YAW_SPEED);
		break;

	case TASK_HECUGRUNT_SIGNAL_BEST_SOUND:
		if (IsInSquad() && GetSquad()->NumMembers() > 1)
		{
			CBasePlayer *pPlayer = AI_GetSinglePlayer();

			if (pPlayer && OccupyStrategySlot(SQUAD_SLOT_EXCLUSIVE_HANDSIGN) && pPlayer->FInViewCone(this) && !IsRecon())
			{
				CSound *pSound;
				pSound = GetBestSound();
	...
	case TASK_ANNOUNCE_ATTACK:
	{
		...
		{
			m_Sentences.Speak("HG_GREN", SENTENCE_PRIORITY_MEDIUM);
			SetActivity(ACT_IDLE);
	...
	case TASK_HECUGRUNT_FACE_TOSS_DIR:
		break;


	case TASK_HECUGRUNT_BEGIN_FLANK:
	{
		if (IsInSquad() && GetSquad()->NumMembers() > 1)
		{
	...
	case TASK_HECUGRUNT_GET_PATH_TO_FORCED_GREN_LOS:
	{
	...
	break;
	case TASK_HECUGRUNT_MOVE_TO_GRENADE_POSITION:
	{
		CBaseEntity *pEnemy = GetEnemy();

		if (GetEnemies()->LastSeenPosition(pEnemy) == NULL)
		{
			TaskFail(FAIL_NO_ENEMY);
			return;
		}

		float flMaxRange = 1200;
		float flMinRange = 200;
		Vector vecAboveEnemy = Vector(0, 0, 80);
		Vector vecEnemy;
		vecEnemy = GetEnemies()->LastSeenPosition(pEnemy) + vecAboveEnemy;  // trying to aim above them for a straight toss
		...
		{
			float dist = (posLos - vecEnemy).Length();
			if (dist < flMaxRange && dist > flMinRange)
				found = true;
		}
	...
	case TASK_HECUGRUNT_FIND_SPREAD_POSITION:
	{
		if (GetEnemy() == NULL)
		{
		...
		else
		{
			...
			Vector vecPos = GetAbsOrigin() + Vector(0, 0, 64) + random->RandomFloat(256, 256) * vecDir;
	...
	case TASK_HECUGRUNT_IGNORE_ATTACKS:
		// must be in a squad
	...
	case TASK_HECUGRUNT_DEFER_SQUAD_GRENADES:
	{
		if (m_pSquad)
		{
			...
			while (pSquadmate)
			{
				...
				if (pCombine)
				{
					pCombine->m_flNextGrenadeCheck = gpGlobals->curtime + 7.5; //5
					pCombine->m_flNextDeployCheck = gpGlobals->curtime + 20.0;
				}
		...
		if (GetHintGroup() == NULL_STRING)
		{
			...
			if (pEntity)
			{
				...
				if (!HasMemory(bits_MEMORY_PLAYER_HURT) && pEntity->IsPlayer() && pEntity->GetHealth() <= 20)
				{
					...
					m_Sentences.Speak("HG_PLAYERHIT", SENTENCE_PRIORITY_INVALID);
	...
	case TASK_HECUGRUNT_DIE_INSTANTLY:
	{
...
int CNPC_HGruntBase::OnTakeDamage_Alive(const CTakeDamageInfo &info)
{
	m_iSoldierdmg++;

	if (info.GetDamageType() & DMG_BULLET && GetCombineDmg() >= 5)
	{
...
void CNPC_HGruntBase::RunTask(const Task_t *pTask)
{
	...
	*/
	...
	case TASK_HECUGRUNT_MOVE_TO_GRENADE_POSITION:
	{
		...
		{
			TaskInterrupt();
			m_flNextGrenadeRunCheck = gpGlobals->curtime + 3.0;
		}
	}
	break;
...
void CNPC_HGruntBase::BuildScheduleTestBits(void)
{
	...
	if (!IsCurSchedule(SCHED_HECUGRUNT_BURNING_STAND))
	{
...
Activity CNPC_HGruntBase::NPC_TranslateActivity(Activity eNewActivity)
{
	...
	//if (eNewActivity == ACT_OPEN_DOOR)
	//{
	//	eNewActivity = ACT_MELEE_ATTACK1;
	//	}

	if (eNewActivity == ACT_RANGE_ATTACK_SHOTGUN)
	{
		eNewActivity = ACT_RANGE_ATTACK_SMG1;
	}

	if (eNewActivity == ACT_RANGE_ATTACK_SHOTGUN_LOW)
	{
	...
	if (eNewActivity == ACT_WALK_AIM_SHOTGUN)
	{
		eNewActivity = ACT_WALK_AIM_RIFLE;
	}

	if (eNewActivity == ACT_RUN_AIM_SHOTGUN)
	{
...
{
	const char *pSentenceName = "HG_MONST";
	switch ( pEnemy->Classify() )
	{
	case CLASS_PLAYER:
		pSentenceName = "HG_ALERT";
		break;
...
{
	m_Sentences.Speak( "HG_HOSTILEDOWN", SENTENCE_PRIORITY_HIGH );
}
...
//	CBaseEntity * pEnemy = GetEnemy();
	...
	if (HasCondition(COND_ENEMY_DEAD))
	{
		...
		if (!HasCondition(COND_SEE_ENEMY) && OccupyStrategySlot(SQUAD_SLOT_GRENADE1) && !IsElite())
			{
				if (CanGrenadeEnemy())
				{
				...
				{
					return SCHED_HECUGRUNT_MOVE_TO_GRENADE_POSITION;
				}
				/**/
			//	return SCHED_HECUGRUNT_TOSS_GRENADE_COVER1;
			}
			else if (CanAltFireEnemy(false) && OccupyStrategySlot(SQUAD_SLOT_SPECIAL_ATTACK))
			{
				return SCHED_HECUGRUNT_AR2_ALTFIRE;
			}

	if (HasCondition(COND_HEAVY_DAMAGE))
		return (SCHED_HECUGRUNT_FLINCH);
	...
	if (HasCondition(COND_NEW_ENEMY))
	{
		...
		if (m_pSquad && pEnemy)
		{
			...
			if (HasCondition(COND_CAN_RANGE_ATTACK1) && OccupyStrategySlot(SQUAD_SLOT_ATTACK1))
			{
				...
				return SCHED_HECUGRUNT_SUPPRESS;
			}
			...
			if (m_pSquad->IsLeader(this) || (m_pSquad->GetLeader() && m_pSquad->GetLeader()->GetEnemy() != pEnemy))
			{
				...
				// This code only runs if the code above didn't assign me SCHED_HECUGRUNT_SUPPRESS.
				if (HasCondition(COND_CAN_RANGE_ATTACK1) && OccupyStrategySlotRange(SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK3))
				{
					return SCHED_RANGE_ATTACK1;
				}

				if (HasCondition(COND_WEAPON_HAS_LOS) && IsStrategySlotRangeOccupied(SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK3))
				{

					// If everyone else is attacking and I have line of fire, flank or wait for a chance to cover someone.

						if (OccupyStrategySlotRange(SQUAD_SLOT_COMBINE_FLANK_FIRST, SQUAD_SLOT_COMBINE_FLANK_LAST))
						{
							//return SCHED_HECUGRUNT_FLANK_ENEMY;
							return	SCHED_HECUGRUNT_TAKE_SPREAD_COVER;
						}
						else
							if (OccupyStrategySlotRange(SQUAD_SLOT_OVERWATCH, SQUAD_SLOT_OVERWATCH4))
							{
								return SCHED_HECUGRUNT_ENTER_OVERWATCH;
							}
			...
			else
			{
				...
				if (!bFirstContact && OccupyStrategySlotRange(SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK2))
				{
					...
					{
						//return SCHED_HECUGRUNT_PRESS_ATTACK;
						return SCHED_HECUGRUNT_SPREAD_OUT;
					}
					/**/
				}
	...
	if (HasCondition(COND_ENEMY_OCCLUDED))
	{
		...
		return SCHED_HECUGRUNT_WAIT_IN_COVER;
	}
	...
	if (HasCondition(COND_SEE_ENEMY) && !HasCondition(COND_CAN_RANGE_ATTACK1))
	{
		...
		AnnounceAssault();
		//return SCHED_HECUGRUNT_ASSAULT;
		return SCHED_TAKE_COVER_FROM_ENEMY;
	}
...
int CNPC_HGruntBase::SelectSchedule(void)
{
	...
	if (HasCondition(COND_COMBINE_ON_FIRE))
		return SCHED_HECUGRUNT_BURNING_STAND;
	...
	if (m_hForcedGrenadeTarget)
	{
		if (m_flNextGrenadeCheck < gpGlobals->curtime)
		{
			...
			else
			{
				...
				{
					m_hForcedGrenadeTarget = NULL;
					return SCHED_HECUGRUNT_FORCED_GRENADE_THROW;
				}
			}
		}
		...
		if (!FVisible(m_hForcedGrenadeTarget))
		{
			return SCHED_HECUGRUNT_MOVE_TO_FORCED_GREN_LOS;
		}
	}
	...
	if (m_NPCState != NPC_STATE_SCRIPT)
	{
		...
		if (HasCondition(COND_COMBINE_HIT_BY_BUGBAIT))
		{
			...
			return SCHED_HECUGRUNT_BUGBAIT_DISTRACTION;
		}
		...
		if (HasCondition(COND_HEAR_MOVE_AWAY))
		{
			...
			return SCHED_HECUGRUNT_TAKE_COVER2;
		}
			...
			if (HasCondition(COND_HEAR_DANGER))
			{
				...
				if (pSound)
				{
					if (pSound->m_iType & SOUND_DANGER)
					{
						...
						m_Sentences.Speak( "HG_COVER", SENTENCE_PRIORITY_NORMAL, SENTENCE_CRITERIA_NORMAL );
						...
						if (!GetEnemy() && pSound->IsSoundType(SOUND_CONTEXT_DANGER_APPROACH) && pSound->m_hOwner && !FInViewCone(pSound->GetSoundReactOrigin()))
						{
							...
							return SCHED_HECUGRUNT_FACE_IDEAL_YAW;
						}

						return SCHED_TAKE_COVER_FROM_BEST_SOUND;
						CSoundEnt::InsertSound(SOUND_DANGER | SOUND_CONTEXT_COMBINE_ONLY, GetAbsOrigin(), 550, 3.5, this);
					}
...
{
	if (failedSchedule == SCHED_HECUGRUNT_TAKE_COVER1)
	{
		if (IsInSquad() && IsStrategySlotRangeOccupied(SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK3) && HasCondition(COND_SEE_ENEMY))
		{
			...
			return SCHED_HECUGRUNT_MOVE_TO_MELEE;
		}
	}
...
int CNPC_HGruntBase::SelectScheduleAttack()
{
	...
	if (HasCondition(COND_COMBINE_DROP_GRENADE))
		return SCHED_HECUGRUNT_DROP_GRENADE;
	...
	if (GetEnemy() && GetEnemy()->Classify() == CLASS_COMBINE && FClassnameIs(GetEnemy(), "npc_turret_floor"))
	{
		...
		if (!GetEnemy()->MyNPCPointer()->FInViewCone(this) && OccupyStrategySlot(SQUAD_SLOT_GRENADE1))
			return SCHED_HECUGRUNT_CHARGE_TURRET;
	}
	...
	if (ShouldChargePlayer())
	{
		...
		{
			return SCHED_HECUGRUNT_CHARGE_PLAYER;
		}
	}
	...
	if (HasCondition(COND_CAN_RANGE_ATTACK1))
	{
		...
		if (OccupyStrategySlotRange(SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK3))
		{
			...
			if (CanGrenadeEnemy() && m_pSquad && m_pSquad->GetSquadMemberNearestTo(GetEnemies()->LastSeenPosition(pEnemy)) == this)
			{
				if (OccupyStrategySlot(SQUAD_SLOT_GRENADE1))
				{
					return SCHED_RANGE_ATTACK2;
	...
	//&& m_pSquad && m_pSquad->GetSquadMemberNearestTo(GetEnemy()->GetAbsOrigin())
	if (GetEnemy() && !HasCondition(COND_SEE_ENEMY))
	{
		...
		//Msg("Time: %f   Dist: %f\n", flTime, flDist );
		if (flTime <= COMBINE_GRENADE_FLUSH_TIME && flDist <= COMBINE_GRENADE_FLUSH_DIST && CanGrenadeEnemy(false) 
			&& OccupyStrategySlot(SQUAD_SLOT_GRENADE1) && !IsStrategySlotRangeOccupied(SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK3))
		{
			return SCHED_RANGE_ATTACK2;
		}
	}
...
int CNPC_HGruntBase::TranslateSchedule(int scheduleType)
{
	...
	case SCHED_TAKE_COVER_FROM_ENEMY:
	{
		if (m_pSquad)
		{
			...
			else
			{
				...
				else if (OccupyStrategySlotRange(SQUAD_SLOT_COMBINE_FLANK_FIRST, SQUAD_SLOT_COMBINE_FLANK_LAST))
				return	SCHED_HECUGRUNT_TAKE_SPREAD_COVER;


				return SCHED_HECUGRUNT_TAKE_COVER1;
			}
		}
		else
		{
			...
			{
				return SCHED_HECUGRUNT_GRENADE_COVER1;
			}
			else
			{
				if (ShouldChargePlayer() && !IsUnreachable(GetEnemy()))
	...
	{
		return SCHED_HECUGRUNT_TAKE_COVER_FROM_BEST_SOUND;
	}
	break;
	case SCHED_HECUGRUNT_TAKECOVER_FAILED:
	{
		if (HasCondition(COND_CAN_RANGE_ATTACK1) && OccupyStrategySlotRange(SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK3))
		{
	...
	case SCHED_HECUGRUNT_TAKE_COVER2:
	{
		if (CanGrenadeEnemy() && OccupyStrategySlot(SQUAD_SLOT_GRENADE1) && !HasCondition(COND_SEE_ENEMY) && GetEnemy())
		{
			return  SCHED_RANGE_ATTACK2;
			//return SCHED_HECUGRUNT_TOSS_GRENADE_COVER1;
		}
	...
	case SCHED_FAIL_ESTABLISH_LINE_OF_FIRE:
	{
		...
		if (!HasCondition(COND_SEE_ENEMY) && OccupyStrategySlot(SQUAD_SLOT_GRENADE1) && !IsElite() && m_flNextGrenadeRunCheck < gpGlobals->curtime)
		{
			return TranslateSchedule(SCHED_HECUGRUNT_MOVE_TO_GRENADE_POSITION);
		}
		/**/
	...
	case SCHED_HECUGRUNT_MOVE_TO_GRENADE_POSITION:
	{
		...
		if (CanGrenadeEnemy(false) && flDist <= COMBINE_GRENADE_FLUSH_DIST && OccupyStrategySlot(SQUAD_SLOT_GRENADE1))
		{
	...
	case SCHED_ESTABLISH_LINE_OF_FIRE:
	{
		...
		float flDist2;
		flDist2 = (GetEnemy()->GetAbsOrigin() - GetAbsOrigin()).Length();
		// always assume standing
		// Stand();

		CBaseEntity * pEnemy = GetEnemy();
		if (CanGrenadeEnemy() && !HasCondition(COND_SEE_ENEMY) && GetEnemy() && OccupyStrategySlot(SQUAD_SLOT_GRENADE1) 
			&& m_pSquad && m_pSquad->GetSquadMemberNearestTo(GetEnemies()->LastKnownPosition(pEnemy)) == this && (GetEnemyLastTimeSeen() < (gpGlobals->curtime + 3.0)))
		{
			return  SCHED_RANGE_ATTACK2;
			//return SCHED_HECUGRUNT_TOSS_GRENADE_COVER1;
		}
		...
		if (OccupyStrategySlotRange(SQUAD_SLOT_COMBINE_FLANK_FIRST, SQUAD_SLOT_COMBINE_FLANK_LAST) && flDist2 >= 200.0f 
			&& IsStrategySlotRangeOccupied(SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK2))
		{
			//return SCHED_HECUGRUNT_SPREAD_OUT;
			return SCHED_HECUGRUNT_TAKE_SPREAD_COVER;
		}
		/**/
		return SCHED_HECUGRUNT_ESTABLISH_LINE_OF_FIRE;
	}
	break;
	case SCHED_HIDE_AND_RELOAD:
	{
		...
		if (CanGrenadeEnemy() && OccupyStrategySlot(SQUAD_SLOT_GRENADE1) && random->RandomInt(0, 100) < 20)
		{
			...
			return SCHED_HECUGRUNT_GRENADE_AND_RELOAD;
		}
		...
		return SCHED_HECUGRUNT_HIDE_AND_RELOAD;
	}
	break;
	case SCHED_RANGE_ATTACK1:
	{
		...
		return SCHED_HECUGRUNT_RANGE_ATTACK1;
	}
	case SCHED_RANGE_ATTACK2:
	{
		...
		// Otherwise use innate attack
			...
			{
				return SCHED_HECUGRUNT_RANGE_ATTACK2;
			}
	}
	// SCHED_COMBAT_FACE:
	// SCHED_HECUGRUNT_WAIT_FACE_ENEMY:
	// SCHED_HECUGRUNT_SWEEP:
	// SCHED_HECUGRUNT_COVER_AND_RELOAD:
	// SCHED_HECUGRUNT_FOUND_ENEMY:

	case SCHED_VICTORY_DANCE:
	{
		return SCHED_HECUGRUNT_VICTORY_DANCE;
	}
	case SCHED_HECUGRUNT_SUPPRESS:
	{
#define MIN_SIGNAL_DIST	256
	...
	case SCHED_FAIL:
	{
		if (GetEnemy() != NULL)
		{
			return SCHED_HECUGRUNT_COMBAT_FAIL;
		}
		return SCHED_FAIL;
	}

	case SCHED_HECUGRUNT_PATROL:
	{
		// If I have an enemy, don't go off into random patrol mode.
		if (GetEnemy() && GetEnemy()->IsAlive())
			//return SCHED_HECUGRUNT_PATROL_ENEMY;
			return SCHED_HECUGRUNT_ENTER_OVERWATCH;
	...
	case SCHED_HECUGRUNT_OVERWATCH:
	{
		if (CanGrenadeEnemy() && !HasCondition(COND_SEE_ENEMY) && GetEnemy() && OccupyStrategySlot(SQUAD_SLOT_GRENADE1) && m_pSquad)
		{
			return  SCHED_RANGE_ATTACK2;
			//return SCHED_HECUGRUNT_TOSS_GRENADE_COVER1;
		}
		else if (CanAltFireEnemy(false) && !HasCondition(COND_SEE_ENEMY) && OccupyStrategySlot(SQUAD_SLOT_SPECIAL_ATTACK))
		{
			return SCHED_HECUGRUNT_AR2_ALTFIRE;
		}

		return SCHED_HECUGRUNT_OVERWATCH;
	}
	break;
	...
	else
	{
		...
		case COMBINE_AE_GREN_TOSS:
		{
			...
			if (m_NPCState == NPC_STATE_SCRIPT)
			{
				...
				Fraggrenade_Create(vecStart + vecNade, vec3_angle, vecThrow, vecSpin, this, COMBINE_GRENADE_TIMER, true);
			}
			else
			{
				// Use the Velocity that AI gave us.
				Fraggrenade_Create(vecStart + vecNade, vec3_angle, m_vecTossVelocity, vecSpin, this, COMBINE_GRENADE_TIMER, true);
				m_iNumGrenades--;
			}

			// wait six seconds before even looking again to see if a grenade can be thrown.
			m_flNextGrenadeCheck = gpGlobals->curtime + 6.0; //6
		}
		handledEvent = true;
		break;
		...
		{
			EmitSound("NPC_HGruntBase.GrenadeLaunch");
			...
			if (g_pGameRules->IsSkillLevel(SKILL_HARD))
				...
				m_flNextGrenadeCheck = gpGlobals->curtime + 6.0;// wait six seconds before even looking again to see if a grenade can be thrown.
		}
		handledEvent = true;
		break;
		...
		case COMBINE_AE_KICK:
		{
			...
			if (pBCC)
			{
				...
				if (!pBCC->DispatchInteraction(i_interactionCombineBash, NULL, this))
				{
					if (pBCC->IsPlayer())
					{
					...
					EmitSound("NPC_HGruntBase.WeaponBash");
				}
			}
...
void CNPC_HGruntBase::LostEnemySound(void)
{
	...
	if ( m_Sentences.Speak( "HG_LOST" ) >= 0 )
	{
...
{
	m_Sentences.Speak( "HG_ALERTH", SENTENCE_PRIORITY_HIGH );
}
...
void CNPC_HGruntBase::NotifyDeadFriend(CBaseEntity* pFriend)
{
	...
	{
		m_Sentences.Speak( "HG_LOST", SENTENCE_PRIORITY_INVALID, SENTENCE_CRITERIA_NORMAL );
		JustMadeSound();
		return;
	}
...
void CNPC_HGruntBase::DeathSound(void)
{
	...
	m_Sentences.Speak( "HG_DIE", SENTENCE_PRIORITY_INVALID, SENTENCE_CRITERIA_ALWAYS );
}
...
void CNPC_HGruntBase::IdleSound(void)
{
	if (g_fHGruntQuestion || random->RandomInt(0,1))
	{
		if (!g_fHGruntQuestion)
		{
			...
			{
			case 0: // check in
				if ( m_Sentences.Speak( "HG_CHECK" ) >= 0 )
				{
					g_fHGruntQuestion = 1;
				}
				break;
			...
			case 1: // question
				if ( m_Sentences.Speak( "HG_QUEST" ) >= 0 )
				{
					g_fHGruntQuestion = 2;
				}
				break;
		...
		else
		{
			...
			{
			case 1: // check in
				if ( m_Sentences.Speak( "HG_CLEAR" ) >= 0 )
				{
					g_fHGruntQuestion = 0;
				}
				break;
			case 2: // question 
				if ( m_Sentences.Speak( "HG_ANSWER" ) >= 0 )
				{
					g_fHGruntQuestion = 0;
				}
				break;
...
bool CNPC_HGruntBase::CanThrowGrenade(const Vector &vecTarget)
{
	...
	flTime = gpGlobals->curtime - GetEnemies()->LastTimeSeen(GetEnemy());
	flDist2 = (GetEnemy()->GetAbsOrigin() - GetEnemies()->LastSeenPosition(GetEnemy())).Length();

	if (flTime > gpGlobals->curtime + 10)
	{
		return false;
	}

	if (flDist2 > COMBINE_GRENADE_FLUSH_DIST)
	{
		return false;
	}
	...
	if (flDist > 1424 || flDist < 256)  //1024 //128
	{
...
bool CNPC_HGruntBase::CheckCanThrowGrenade(const Vector &vecTarget)
{
	...
	vecEnemy = GetEnemies()->LastKnownPosition(pEnemy) + (pEnemy->GetViewOffset()*0.75);// approximates the chest
	vecEnemy2 = GetEnemies()->LastSeenPosition(pEnemy) + vecAboveEnemy;  // trying to aim above cover for a straight toss

	if (FInViewCone(vecTarget) && CBaseEntity::FVisible(vecTarget))
	{
		vecToss = VecCheckThrow(this, EyePosition(), vecTarget, COMBINE_GRENADE_THROW_SPEED, 1.2, &vecMins, &vecMaxs);
	}
	else if (FInViewCone(vecEnemy2) && CBaseEntity::FVisible(vecEnemy2))
	{
		vecToss = VecCheckThrow(this, EyePosition(), vecEnemy2, COMBINE_GRENADE_THROW_SPEED, 1.2, &vecMins, &vecMaxs);
	}

	//GetEnemyLastTimeSeen() > (gpGlobals->curtime - 5.0)
	else
	{
		// Have to try a high toss. Do I have enough room?
		trace_t tr;
		AI_TraceLine(EyePosition(), EyePosition() + Vector(0, 0, 192), MASK_SHOT, this, COLLISION_GROUP_NONE, &tr);
		if (tr.fraction != 1.0 || HasCondition(COND_CANNOT_THROW))
		{
			return false;
		}


		if (flDistSq > Square(400))
		{
			vecToss = VecCheckToss(this, EyePosition(), vecEnemy, -1, 1.2, true, &vecMins, &vecMaxs);
		}
		else
		{
			vecToss = VecCheckToss(this, EyePosition(), vecEnemy, -1, 1.0, true, &vecMins, &vecMaxs);
		}
	}
	...
	if (vecToss != vec3_origin)
	{
		...
		m_flNextGrenadeCheck = gpGlobals->curtime + 3; // 1/3 second.
		return true;
	}
	else
	{
...
bool CNPC_HGruntBase::CanAltFireEnemy(bool bUseFreeKnowledge)
{
	...
	Vector vecTarget;
	Vector vecAbove = Vector(0, 0, 70);

	// Determine what point we're shooting at
	if (bUseFreeKnowledge)
	{
		vecTarget = GetEnemies()->LastKnownPosition(pEnemy) + (pEnemy->GetViewOffset()*0.25);// approximates the feet, aiming low now to make use of the aoe explosion!
	}
	else
	{
		vecTarget = GetEnemies()->LastSeenPosition(pEnemy) + vecAbove;
	}


	//Taken from the cangrenade function now that the altfire blows dudes the fuck up
	/*if (m_pSquad)
	{
		if (m_pSquad->SquadMemberInRange(vecTarget, COMBINE_MIN_GRENADE_CLEAR_DIST))
		{
			...
			// Tell my squad members to clear out so I can get fire an orb
			CSoundEnt::InsertSound(SOUND_MOVE_AWAY | SOUND_CONTEXT_COMBINE_ONLY, vecTarget, COMBINE_MIN_GRENADE_CLEAR_DIST, 0.2);
			return false;
		}
	}*/
	...
	Vector mins(-18, -18, -18);
	Vector maxs(18, 18, 18);
	...
	if (tr.fraction >= 0.65 && flLength > 256.0f)
	{
		// Target is valid
		m_vecAltFireTarget = vecTarget;
		return true;
...
bool CNPC_HGruntBase::OnBeginMoveAndShoot()
{
	...
	{
		if (HasStrategySlotRange(SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK3))
			return true; // already have the slot I need

		if (!HasStrategySlotRange(SQUAD_SLOT_GRENADE1, SQUAD_SLOT_ATTACK_OCCLUDER) && OccupyStrategySlotRange(SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK3))
			return true;
	}
	return false;
}
...
bool CNPC_HGruntBase::IsRunningApproachEnemySchedule()
{
	...
	if (IsCurSchedule(SCHED_HECUGRUNT_PRESS_ATTACK, false))
		return true;
...
AI_BEGIN_CUSTOM_NPC(NPC_HGruntBase, CNPC_HGruntBase)

//Tasks
DECLARE_TASK(TASK_HECUGRUNT_FACE_TOSS_DIR)
DECLARE_TASK(TASK_HECUGRUNT_IGNORE_ATTACKS)
DECLARE_TASK(TASK_HECUGRUNT_SIGNAL_BEST_SOUND)
DECLARE_TASK(TASK_HECUGRUNT_DEFER_SQUAD_GRENADES)
DECLARE_TASK(TASK_HECUGRUNT_CHASE_ENEMY_CONTINUOUSLY)
DECLARE_TASK(TASK_HECUGRUNT_DIE_INSTANTLY)
DECLARE_TASK(TASK_HECUGRUNT_PLAY_SEQUENCE_FACE_ALTFIRE_TARGET)
DECLARE_TASK(TASK_HECUGRUNT_GET_PATH_TO_FORCED_GREN_LOS)
DECLARE_TASK(TASK_HECUGRUNT_SET_STANDING)
DECLARE_TASK(TASK_HECUGRUNT_BEGIN_FLANK)
DECLARE_TASK(TASK_HECUGRUNT_MOVE_TO_GRENADE_POSITION)
DECLARE_TASK(TASK_HECUGRUNT_FIND_SPREAD_POSITION)
...
DECLARE_ACTIVITY(ACT_STRIKER_JUMP)
DECLARE_ACTIVITY(ACT_STRIKER_LAND)
DECLARE_ACTIVITY(ACT_STRIKER_GLIDE)

DECLARE_ANIMEVENT(COMBINE_AE_BEGIN_ALTFIRE)
DECLARE_ANIMEVENT(COMBINE_AE_ALTFIRE)
DECLARE_ANIMEVENT(AE_COMBINE_START_DEPLOY)

DECLARE_SQUADSLOT(SQUAD_SLOT_GRENADE1)
DECLARE_SQUADSLOT(SQUAD_SLOT_GRENADE2)
DECLARE_SQUADSLOT(SQUAD_SLOT_COMBINE_FLANK_FIRST)
...
DECLARE_INTERACTION(i_interactionCombineBash);
...
"	Tasks"
"		 TASK_SET_FAIL_SCHEDULE				SCHEDULE:SCHED_HECUGRUNT_RUN_AWAY_FROM_BEST_SOUND"
"		 TASK_STOP_MOVING					0"
"		 TASK_HECUGRUNT_SIGNAL_BEST_SOUND		0"
...
"		TASK_STOP_MOVING						0"
"		TASK_HECUGRUNT_BEGIN_FLANK					0"
"		TASK_SET_TOLERANCE_DISTANCE				160"
"		TASK_SET_ROUTE_SEARCH_TIME				1"	// Spend 1 second trying to build a path if stuck
"		TASK_GET_FLANK_ARC_PATH_TO_ENEMY_LOS	70"
"		TASK_RUN_PATH							0"
"		TASK_WAIT_FOR_MOVEMENT					0"
"		TASK_FACE_ENEMY							0"
""
...
"	Tasks"
"		TASK_STOP_MOVING					1"
// "		TASK_SET_ACTIVITY					ACTIVITY:ACT_BIG_FLINCH"
"		TASK_SET_ACTIVITY					ACTIVITY:ACT_GESTURE_BIG_FLINCH"
"		TASK_SET_SCHEDULE				SCHEDULE:SCHED_HECUGRUNT_FLINCH_END"
//"		TASK_WAIT							0.25"					// repick IDLESTAND every two seconds."
""
"	Interrupts"
);
...
"	Tasks"
"		TASK_STOP_MOVING					0"
"		TASK_WAIT							0.75"					// unable to attack or move."
""
"	Interrupts"
);
...
"		 TASK_GET_PATH_AWAY_FROM_BEST_SOUND		700"
...
"	Tasks "
"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_HECUGRUNT_ESTABLISH_LINE_OF_FIRE"
"		TASK_SET_TOLERANCE_DISTANCE		256" //48
"		TASK_GET_PATH_TO_ENEMY_LKP		0"
"		TASK_HECUGRUNT_IGNORE_ATTACKS		0.2"
"		TASK_SPEAK_SENTENCE				0"
"		TASK_RUN_PATH					0"
//		"		TASK_HECUGRUNT_MOVE_AND_AIM		0"
"		TASK_WAIT_FOR_MOVEMENT			0"
"		TASK_HECUGRUNT_IGNORE_ATTACKS		0.0"
""
"	Interrupts "
"		COND_NEW_ENEMY"
"		COND_ENEMY_DEAD"
...
"	Tasks "
"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_HECUGRUNT_ESTABLISH_LINE_OF_FIRE"
"		TASK_SET_TOLERANCE_DISTANCE		250" //72
"		TASK_GET_PATH_TO_ENEMY_LKP		0"
"		TASK_HECUGRUNT_SET_STANDING		1"
"		TASK_RUN_PATH					0"
"		TASK_WAIT_FOR_MOVEMENT			0"
...
"		TASK_FACE_ENEMY				0"
"		 TASK_WAIT					1.5"
//"		 TASK_SET_SCHEDULE			SCHEDULE:SCHED_HECUGRUNT_SWEEP"
""
"	Interrupts"
"		COND_NEW_ENEMY"
"		COND_ENEMY_DEAD"
...
"		TASK_SET_ACTIVITY			ACTIVITY:ACT_IDLE"
"		TASK_FACE_ENEMY				0"
"		TASK_SET_SCHEDULE			SCHEDULE:SCHED_HECUGRUNT_OVERWATCH"
""
"	Interrupts"
"		COND_HEAR_DANGER"
"		COND_NEW_ENEMY"
...
"	Tasks"
"		TASK_SET_FAIL_SCHEDULE		SCHEDULE:SCHED_HECUGRUNT_TAKECOVER_FAILED"
"		TASK_STOP_MOVING				0"
"		TASK_WAIT					0.2"
"		TASK_FIND_COVER_FROM_ENEMY	0"
"		TASK_RUN_PATH				0"
...
"		TASK_WAIT					1.2"
"		TASK_REMEMBER				MEMORY:INCOVER"
"		TASK_SET_SCHEDULE			SCHEDULE:SCHED_HECUGRUNT_WAIT_IN_COVER"
//"		TASK_SET_SCHEDULE						SCHEDULE:SCHED_HECUGRUNT_SPREAD_OUT"
""
"	Interrupts"
)
...
"		TASK_WAIT					0.2"
"		TASK_FIND_COVER_FROM_ENEMY			0"
"		TASK_FIND_FAR_NODE_COVER_FROM_ENEMY	420"
"		TASK_RUN_PATH				0"
"		TASK_WAIT_FOR_MOVEMENT		0"
"		TASK_REMEMBER				MEMORY:INCOVER"
"		TASK_SET_SCHEDULE			SCHEDULE:SCHED_HECUGRUNT_WAIT_IN_COVER2"
""
"	Interrupts"
//"		COND_HEAR_DANGER"
)
...
"	Tasks"
//"		TASK_SET_FAIL_SCHEDULE		SCHEDULE:SCHED_HECUGRUNT_SPREAD_OUT"
"		TASK_SET_FAIL_SCHEDULE		SCHEDULE:SCHED_HECUGRUNT_TAKECOVER_FAILED"
"		TASK_STOP_MOVING				0"
"		TASK_WAIT					0.2"
"		TASK_FIND_FAR_NODE_COVER_FROM_ENEMY	256"
"		TASK_FIND_NEAR_NODE_COVER_FROM_ENEMY 400"	
"		TASK_RUN_PATH				0"
"		TASK_WAIT_FOR_MOVEMENT		0"
"		TASK_WAIT					0.2"
"		TASK_REMEMBER				MEMORY:INCOVER"
"		TASK_SET_SCHEDULE						SCHEDULE:SCHED_HECUGRUNT_FLANK_ENEMY"
//"		TASK_SET_SCHEDULE						SCHEDULE:SCHED_HECUGRUNT_SPREAD_OUT"
""
"	Interrupts"
)
...
"      TASK_HECUGRUNT_DEFER_SQUAD_GRENADES   0"
""
"	Interrupts"
)
...
"		TASK_RANGE_ATTACK1				0"
//"		TASK_HECUGRUNT_IGNORE_ATTACKS		0.5"
""
"	Interrupts"
"		COND_NEW_ENEMY"
"		COND_ENEMY_DEAD"
...
"		TASK_HECUGRUNT_PLAY_SEQUENCE_FACE_ALTFIRE_TARGET		ACTIVITY:ACT_COMBINE_AR2_ALTFIRE"
""
"	Interrupts"
)
...
"	Tasks"
"		TASK_STOP_MOVING					0"
"		TASK_HECUGRUNT_FACE_TOSS_DIR			0"
...
"	Tasks "
"		TASK_SET_TOLERANCE_DISTANCE					48"
"		TASK_HECUGRUNT_GET_PATH_TO_FORCED_GREN_LOS	0"
"		TASK_SPEAK_SENTENCE							1"
"		TASK_RUN_PATH								0"
"		TASK_WAIT_FOR_MOVEMENT						0"
"	"
...
"	Tasks"
//"		TASK_SET_FAIL_SCHEDULE					SCHEDULE:SCHED_HECUGRUNT_FLANK_ENEMY"
"		TASK_SET_ROUTE_SEARCH_TIME				1"	// Spend 1 second trying to build a path if stuck
//"		TASK_MOVE_AWAY_PATH						284"
"		TASK_HECUGRUNT_FIND_SPREAD_POSITION		0"
...
"	Tasks "
"		TASK_SET_TOLERANCE_DISTANCE					1200"
"		TASK_HECUGRUNT_MOVE_TO_GRENADE_POSITION	0"
"		TASK_SPEAK_SENTENCE							1"
"		TASK_RUN_PATH								0"
"		TASK_WAIT_FOR_MOVEMENT						0"
"	"
...
"		TASK_PLAY_SEQUENCE					ACTIVITY:ACT_RANGE_ATTACK2"
"		TASK_HECUGRUNT_DEFER_SQUAD_GRENADES	0"
"		TASK_SET_SCHEDULE					SCHEDULE:SCHED_HECUGRUNT_WAIT_IN_COVER"	// don't run immediately after throwing grenade.
""
"	Interrupts"
)
...
"		TASK_WAIT						3"
"		TASK_WAIT_RANDOM				3"
"		TASK_SET_SCHEDULE				SCHEDULE:SCHED_HECUGRUNT_PATROL" // keep doing it
""
"	Interrupts"
"		COND_ENEMY_DEAD"
"		COND_LIGHT_DAMAGE"
...
"		TASK_STOP_MOVING					0"
"		TASK_SET_FAIL_SCHEDULE				SCHEDULE:SCHED_CHASE_ENEMY_FAILED"
"		TASK_HECUGRUNT_CHASE_ENEMY_CONTINUOUSLY		192"
"		TASK_FACE_ENEMY						0"
""
"	Interrupts"
"		COND_NEW_ENEMY"
...
//
// Used instead if SCHED_HECUGRUNT_PATROL if I have an enemy.
...
"		TASK_WAIT						2"
"		TASK_WAIT_RANDOM				3"
"		TASK_HECUGRUNT_DIE_INSTANTLY		DMG_BURN"
"		TASK_WAIT						1.0"
"	"
"	Interrupts"
)
