...
acttable_t	CWeaponBugSpawner::m_acttable[] = 
{
	{ ACT_RANGE_ATTACK1, ACT_RANGE_ATTACK_THROW, true },
};
IMPLEMENT_ACTTABLE(CWeaponBugSpawner);
...
// Output  :
//------------------------------------------------------------------------------
void CWeaponBugSpawner::SetPickupTouch( void )
{
	SetTouch( &CWeaponBugSpawner::MolotovTouch );
}
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponBugSpawner::MolotovTouch( CBaseEntity *pOther )
{
	// ---------------------------------------------------
	...
	if (pOther->GetFlags() & FL_CLIENT)
	{
		...
		CWeaponBugSpawner* oldWeapon = (CWeaponBugSpawner*)pBCC->Weapon_OwnsThisType( GetClassname() );
		if (oldWeapon != this)
		{
			UTIL_Remove( this );
		}
...
// Output :
//-----------------------------------------------------------------------------
bool CWeaponBugSpawner::ObjectInWay( void )
{
...
// Output :
//-----------------------------------------------------------------------------
bool CWeaponBugSpawner::WeaponLOSCondition(const Vector &ownerPos, const Vector &targetPos,bool bSetConditions)
{
...
// Output :
//-----------------------------------------------------------------------------
int CWeaponBugSpawner::WeaponRangeAttack1Condition( float flDot, float flDist )
{
...
//
//-----------------------------------------------------------------------------
void CWeaponBugSpawner::ThrowMolotov( const Vector &vecSrc, const Vector &vecVelocity)
{
	CGrenadeBugSpawner *pMolotov = (CGrenadeBugSpawner*)Create( "grenade_bugspawner", vecSrc, vec3_angle, GetOwner() );

	if (!pMolotov)
	{
		Msg("Couldn't make bugspawner!\n");
		return;
	}
...
//
//-----------------------------------------------------------------------------
void CWeaponBugSpawner::DrawAmmo( void )
{
	// -------------------------------------------
