...
#include "smod_motionblur_ps20.inc"
#include "smod_motionblur_ps20b.inc"
#include "convar.h"

// memdbgon must be the last include file in a .cpp file!!!
#include "tier0/memdbgon.h"

extern ConVar mat_motion_blur_percent_of_screen_max;

DEFINE_FALLBACK_SHADER(SDK_MotionBlur, SDK_MotionBlur_dx9)
BEGIN_VS_SHADER_FLAGS(SDK_MotionBlur_dx9, "Motion Blur", SHADER_NOT_EDITABLE)
BEGIN_SHADER_PARAMS
...
{
	DECLARE_STATIC_PIXEL_SHADER(smod_motionblur_ps20b);
	SET_STATIC_PIXEL_SHADER(smod_motionblur_ps20b);
}
else
{
	DECLARE_STATIC_PIXEL_SHADER(smod_motionblur_ps20);
	SET_STATIC_PIXEL_SHADER(smod_motionblur_ps20);
}
...
if (g_pHardwareConfig->SupportsPixelShaders_2_b())
{
	DECLARE_DYNAMIC_PIXEL_SHADER(smod_motionblur_ps20b);
	SET_DYNAMIC_PIXEL_SHADER_COMBO(QUALITY, nQuality);
	SET_DYNAMIC_PIXEL_SHADER(smod_motionblur_ps20b);
}
else
{
	DECLARE_DYNAMIC_PIXEL_SHADER(smod_motionblur_ps20);
	SET_DYNAMIC_PIXEL_SHADER_COMBO(QUALITY, nQuality);
	SET_DYNAMIC_PIXEL_SHADER(smod_motionblur_ps20);
}
}
