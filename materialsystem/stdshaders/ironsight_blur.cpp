...
#include "smod_ironsight_blur_ps20b.inc"

BEGIN_VS_SHADER_FLAGS(smod_ironsight_blur, "Custom ironsight_blur shader", SHADER_NOT_EDITABLE)
BEGIN_SHADER_PARAMS
...
{
	DECLARE_STATIC_PIXEL_SHADER(smod_ironsight_blur_ps20b);
	SET_STATIC_PIXEL_SHADER(smod_ironsight_blur_ps20b);
}
}
...
if (g_pHardwareConfig->SupportsPixelShaders_2_b())
{
	DECLARE_DYNAMIC_PIXEL_SHADER(smod_ironsight_blur_ps20b);
	SET_DYNAMIC_PIXEL_SHADER(smod_ironsight_blur_ps20b);
}
}
Draw();
}
