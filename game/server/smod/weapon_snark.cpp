...
//LINK_ENTITY_TO_CLASS( weapon_snark, CWeaponSnarkHD );

//PRECACHE_WEAPON_REGISTER( weapon_snark );

IMPLEMENT_SERVERCLASS_ST( CWeaponSnarkHD, DT_WeaponSnarkHD )
END_SEND_TABLE()

BEGIN_DATADESC( CWeaponSnarkHD )
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CWeaponSnarkHD::CWeaponSnarkHD( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponSnarkHD::Precache( void )
{
	BaseClass::Precache();
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponSnarkHD::PrimaryAttack( void )
{
...
void CWeaponSnarkHD::WeaponIdle( void )
{
	//SMOD: Ironsight fix
	if (m_bIsIronsighted)
		return;
...
bool CWeaponSnarkHD::Deploy( void )
{
	CPASAttenuationFilter filter( this );
...
bool CWeaponSnarkHD::Holster( CBaseCombatWeapon *pSwitchingTo )
{
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
	if ( !pPlayer )
	{
	...
	if ( pPlayer->GetAmmoCount( m_iPrimaryAmmoType ) <= 0 )
	{
		SetThink( &CWeaponSnarkHD::DestroyItem );
